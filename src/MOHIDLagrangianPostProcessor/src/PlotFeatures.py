...
def group_resample(da: xr.DataArray, group_type: str, time_freq: str) -> xr.DataArray:
    """
...
def isgroupable(da: xr.DataArray, group_type: str, time_freq: str, method: str) -> bool:
    """
...
def weight_dataarray_with_csv(
    dataArray: xr.DataArray, weight_file: str
) -> xr.DataArray:
    """
...
def get_background_map(ax, extent):
    ...
    if extent_width > 2:  # one degree ~ 111 km
        land_10m = cfeature.NaturalEarthFeature(
            "physical", "land", "10m", edgecolor="face", facecolor=gray_color_RGB
        )
...
def get_color_lims(
    dataArray: xr.DataArray, robust: bool = True, min_quartile=0.05, max_quartile=0.99
):
    """
...
def get_horizontal_scale(dataArray: xr.DataArray) -> np.int64:
    """Get the horizontal scale to build the scale bar.

    It approaches 1-100km to tenth part of horizontal extent
...
def get_title_methods(methods: list, variable: str, debug_title: bool = True) -> str:
    """get title methods
...
class Normalizer:
    ...
    def getMeanStdGlobal(dataset, dim=None):
        if dim is None:
            ...
            return dataset["n_counts_global"].mean(dim=dim), dataset[
                "n_counts_global"
            ].std(dim=dim)
    ...
    def getNormalizedDataArray(self, da):
        if self.method == "total":
            ...
            # is done
            da.values = (da.values / self.factor) * 100.0
        elif (self.method == "mean") or (self.method == "mean-zonal"):
            da = da / self.factor[0]
        elif self.method == "max":
